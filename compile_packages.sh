...
cp -r ${WORK_FOLDER}/btulz.transforms.shell/src/main/commands/btulz.shell.bat.txt ${WORK_FOLDER}/release/btulz.shell.bat
chmod 777 ${WORK_FOLDER}/release/btulz.shell.sh
echo --压缩编译文件为tar包
if [ -e ${WORK_FOLDER}/release ]
then
  cd ${WORK_FOLDER}/release/
  tar -cvf btulz.transforms.tar btulz.transforms.* bobas.businessobjectscommon-* jaxen-* log4j-* mysql-connector-java-* ngdbc-* postgresql-* sqljdbc* mssql-jdbc-*.jar jconn4-*.jar dom4j-* btulz.shell.*
fi
cd ${WORK_FOLDER}
